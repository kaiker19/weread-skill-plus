...
- 纯增量覆盖：同名书会被新内容覆盖，重跑安全（同一本书重导用同一文件名，不会产生重复文件）。
- ⚠️ 导出目录是「只读镜像」：每次重导会覆盖同名文件，**别在导出的 `.md` 里直接加批注**，否则下次重导会丢。要批注请在 vault 别处用 `[[书名]]` 链接 / 引用它们。
...
- **默认只同步「读完」的书**（有 `finish_time`）：ima 无法更新，未读完的书划线还会变，导进去就是过时副本、且删不掉。读完的书划线稳定、导一次即最终版。要连在读的一起导用 `--all`。
