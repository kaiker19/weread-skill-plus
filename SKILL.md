...
- 用户说「存进 ima / 同步到 ima 知识库 / 导入 ima」→ `python3 cli/weread.py ima --kb <知识库名>`（默认只导读完的书、增量、防重复；需 ima 凭证 + 先在 ima 客户端建好知识库）
...
## 非 Agent 出口：CLI 与本地面板

以上能力都经 Agent 调度。`lib/` 这套检索/知识库还有两个**不经过 Agent**的出口，复用同一份相似度逻辑、同一个 `data/knowledge.db`，零 LLM 依赖：

**CLI —— 本机即用，可嵌入任意编辑器/启动器**

```bash
python3 cli/weread.py recall "<想法或概念>"     # markdown：相关划线 + 概念
python3 cli/weread.py recall "<...>" --json      # 结构化输出，供脚本/插件消费

python3 cli/weread.py concepts                   # markdown：概念地图（概念簇 + 最强关联）
python3 cli/weread.py concepts --extract         # 先用 data/llm.json 的 LLM 给缺概念的书抽概念，再渲染
...
python3 cli/weread.py ima --kb <知识库名>          # 增量同步「读完」的书到 ima 知识库（防重复、限速；需 ima 凭证）
```
...
软链到 PATH（`ln -s …/cli/weread.py /usr/local/bin/weread`）即成 `weread` 命令，可被 Alfred / Raycast / Obsidian(Shell commands) / vim(`:r !weread recall`) 等 shell-out 调用，`| pbcopy` 进剪贴板。

**本地 Web 面板** —— 可选图形界面（书架 / 探索 / 写作台 / 洞见）。启动方式与各页用法见 [references/web.md](references/web.md)。